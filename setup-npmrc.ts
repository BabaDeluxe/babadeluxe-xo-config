...
function applyEnvFile(
  ...
  if (!existsSync(filePath)) {
    console.warn(`· ${label} not found, skipping → ${filePath}`)
    return target
  }

  const myEnv = parseEnv(readFileSync(filePath, 'utf8'))
...
if (missing.length > 0) {
  throw new SetupError(
    `Missing required env vars: ${missing.join(', ')}. Add them to .env or .env.local, or export before running.`
  )
}
